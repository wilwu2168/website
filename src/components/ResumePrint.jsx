...
export function openResumeForPrint() {
  ...
  const skills = portfolioSections.find((s) => s.type === 'skills')
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .skill-label { font-size: 12px; font-weight: 600; color: #444; }
  .skill-items { font-size: 12.5px; color: #333; }
  @media print {
...
<body>
  ...
  <div class="contact-row">
    ${contact ? `${contact.location} &middot; <a href="mailto:${contact.email}">${contact.email}</a> &middot; <a href="https://${contact.linkedin}">${contact.linkedin}</a>` : ''}
  </div>
  ...
  <div class="skill-cat">
    ...
    <span class="skill-items"> ${cat.items.join(', ')}</span>
  </div>
  `).join('')}
  ` : ''}
</body>
